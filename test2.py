...
def main_structure(inp):
	with tf.variable_scope('mainModel'):
		...
		mod.convLayer(3,128,activation=M.PARAM_LRELU)#32_8x8
		mod.convLayer(3,256,activation=M.PARAM_LRELU)
		mod.flatten()
		mod.fcLayer(256,activation=M.PARAM_RELU)
		mod.fcLayer(1)
...
def build_graph():
	...
	with tf.name_scope('lab_holder'):
		lab_holder = tf.placeholder(tf.float32,[None,1])
...
with tf.Session() as sess:
	...
	for iteration in range(MAXITER):
		...
		# print(iteration)
		sample = []
		j=random.randint(0, 10222)
		sample = data[j]
		img_batch = np.reshape(sample[0],[1,256,256,3])
		lab_batch = np.reshape(sample[1],[1,1])
		tf.cast(sample[0], tf.float32)
		tf.cast(sample[1], tf.float32)
